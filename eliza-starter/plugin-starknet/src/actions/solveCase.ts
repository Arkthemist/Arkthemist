...
export function convertToDeployData(content: any): DeployData | null {
    ...
    const symbol = content["Token Symbol"] || content["tokenSymbol"] || content["tokensymbol"] || content["symbol"] || content["token_symbol"];
    const owner = content["Token Owner"] || content["tokenOwner"] || content["tokenowner"] || content["owner"] || content["token_owner"];
    const initialSupply = content["Token Initial Supply"] || content["tokenInitialSupply"] || content["tokeninitialsupply"] || content["initialSupply"] || content["token_initial_supply"];

    if (typeof name === "string" && typeof symbol === "string" && typeof owner === "string" && typeof initialSupply === "string") {
        return {
            name,
            symbol,
            owner,
