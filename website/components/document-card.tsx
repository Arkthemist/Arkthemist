...
export const DocumentCard = ({ doc, setSelectedDocument, setIsModalOpen }: DocumentCardProps) => {
  if (!doc) {
    return null
  }
  ...
  return (
    <Card key={doc._id}>
          ...
          <Badge variant="secondary">
            {(() => {
              switch (doc?.case_status) {
                case 'pending-lawyer':
                  return "Awaiting lawyer's signature";
      ...
      <CardFooter>
        ...
        >
          {doc.case_status === 'pending-signature' ? 'Review and sign' : 
